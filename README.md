...
Note that Discord Interpreter creates a server vulnurability, since it depends on the exec function. You can spit the output of .bashrc with it by creating a prompt. Essentially, the Ai can access all files in a sandbox. This is why Discord Interpreter is aimed for small study groups and small teams.
